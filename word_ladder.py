...
class Solution(object):
    ...
    def get_match_word_list(self, word):
        word_len = len(word)
        match_word_list = []
        for i in range(word_len):
            for j in range(26):
                match_word = map(lambda x: x, word)
                match_word[i] = alphabet[j]
    ...
    def ladderLength(self, begin_word, end_word, word_list):
        print "Word list len: ", len(word_list)
        """
        :type begin_word: str
        :type end_word: str
        ...
        if end_word not in word_list:
            return 0
        word_list = filter(lambda x: x <> begin_word, word_list)
        word_list = set(word_list) 
        word_list.add(end_word)
        #origin_word_list = set(copy.deepcopy(word_list))
        origin_word_list = word_list
        step_word_list = set([begin_word])
        exist_word_list = set([begin_word])
        steps = 0
        while step_word_list:
            steps += 1
            new_step_word_list = set()
            for curr_word in step_word_list:
                curr_match_word_list = set(self.get_match_word_list(curr_word))
                if end_word in curr_match_word_list:
                    print "GETTT"
                    return steps + 1
                new_step_word_list = new_step_word_list | curr_match_word_list
            #new_step_word_list = set(filter(lambda x: x not in exist_word_list and x in origin_word_list, new_step_word_list))   
            new_step_word_list = new_step_word_list - exist_word_list
            new_step_word_list = new_step_word_list & origin_word_list
            print "step: %s, count: %s" % (steps, len(new_step_word_list))
            step_word_list = new_step_word_list
            exist_word_list = exist_word_list | new_step_word_list
        return 0
...
class TestFunc(unittest.TestCase):
    ...
    def test_1(self):
        ...
        word_list = ["hot","dot","dog","lot","log","cog"]
        steps = 5
        self.assertEqual(steps, Solution().ladderLength(begin_word, end_word, word_list))
